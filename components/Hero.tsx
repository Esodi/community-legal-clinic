...
function FloatingPaths({ position }: { position: number }) {
  ...
  return (
    <div className="absolute inset-0 pointer-events-none">
      <svg className="w-full h-full text-slate-950/10 dark:text-white/10" viewBox="0 0 696 316" fill="none">
...
export default function Hero({ data }: { data: HeroDataType }) {
  // Removing the toggle state since we're only showing AnimatedChat
  ...
  return (
    <section className="relative bg-[#001a42] text-white overflow-hidden pt-24">
      ...
      <div className="max-w-6xl mx-auto px-4 sm:px-6 md:px-8 relative">
        <div className="flex flex-col lg:grid lg:grid-cols-2 gap-8 lg:gap-8 items-start pt-8 lg:pt-12">
          {/* Text content */}
          <div className="relative z-10">
            ...
            <motion.p 
              className="text-base sm:text-lg mb-6 lg:mb-8 max-w-2xl relative z-10 drop-shadow-lg"
              initial={{ opacity: 0 }}
              animate={{ opacity: 1 }}
          ...
          >
            ...
            <div className="hidden lg:block relative h-[600px] xl:h-[700px]">
              <AnimatePresence mode="wait">
                ...
                {/* Keeping phone image code commented out for future use */}
                {/* <motion.div
                  key="phone"
                  initial={{ opacity: 0, scale: 0.95 }}
                  animate={{ opacity: 1, scale: 1 }}
