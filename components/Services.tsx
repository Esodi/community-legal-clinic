...
export default function Services({ data }: ServicesProps) {
  ...
  useEffect(() => {
    const handleResize = () => {
      ...
      setItemsPerPage(width >= 1280 ? 3 : width >= 1024 ? 3 : width >= 640 ? 2 : 1);
    };
    handleResize();
  ...
  const prevSlide = () => setCurrentIndex((i) => (i - 1 + totalPages) % totalPages);
    ...
    >
      ...
      >
        ...
        <div className="relative">
          ...
          <button
            onClick={prevSlide}
            className="absolute left-0 top-1/2 -translate-y-1/2 z-20 p-3 rounded-full bg-white/10 hover:bg-[#071a42] transition-colors shadow-lg hidden md:flex items-center justify-center"
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4 lg:gap-5 mt-8">
            <AnimatePresence mode="sync">
              {visibleServices.map((service, idx) => (
                ...
                >
                  <div className="bg-[#f8faff] rounded-lg overflow-hidden shadow-lg flex flex-col h-full border border-gray-100 dark:border-gray-800 hover:shadow-xl transition-all duration-300">
                    {/* Image */}
                    <div
                    ...
                    {/* Content */}
                    <div className="p-3 lg:p-4 flex-1 flex flex-col h-full">
                      <div className="h-[120px] lg:h-[100px] overflow-hidden">
                        <h3 className="text-gray-900 text-lg lg:text-xl font-semibold mb-1 lg:mb-2 line-clamp-2">
                          {service.title}
                        </h3>
                      ...
                      {/* Offerings */}
                      <div className="bg-gray-50 p-2 lg:p-3 rounded-lg shadow-sm border border-gray-300 mt-2">
                        <div className="flex items-center mb-1 lg:mb-2">
                          <Cog className="h-4 w-4 lg:h-5 lg:w-5 mr-1 text-gray-900 flex-shrink-0" />
                          <h4 className="text-gray-900 text-sm lg:text-base font-semibold">What We Offer:</h4>
                        </div>
                        <div className="space-y-1 lg:space-y-2">
                          {service.offerings.slice(0, 2).map((offering, i) => (
                            <div key={offering.id} className="flex items-start gap-2">
                              <div className="flex-shrink-0 w-4 h-4 lg:w-5 lg:h-5 rounded-full border border-blue-500/30 flex items-center justify-center bg-white">
                                <span className="text-blue-600 text-xs font-semibold">{i + 1}</span>
                              </div>
                              <p className="text-gray-600 text-xs lg:text-sm leading-tight flex-1">{offering.text}</p>
                            </div>
                          ))}
                        </div>
                      </div>
          ...
          <button
            onClick={nextSlide}
            className="absolute right-0 top-1/2 -translate-y-1/2 z-20 p-3 rounded-full bg-white/10 hover:bg-[#071a42] transition-colors shadow-lg hidden md:flex items-center justify-center"
        ...
        {/* Mobile nav buttons */}
        <div className="flex md:hidden justify-center gap-4 mt-8 mb-4">
          <button
            onClick={prevSlide}
            className="w-12 h-12 rounded-full bg-blue-700 text-white hover:bg-blue-800 flex items-center justify-center shadow-md mx-2"
            aria-label="Previous slide"
          >
